...
| Corridor | `POST` | `/incidents/{incident_id}/corridor` | `CorridorGenerateRequest` | `CorridorRead` | None | 404, 409, 422 | Computes Green Wave | Map / Route Actions | Phase 03 |
| Corridor | `POST` | `/incidents/{incident_id}/corridor/priority` | `CorridorPriorityRequest` | `CorridorRead` | `expected_incident_version`, `expected_plan_version` | 404, 409, 422 | Signal gateway push | Signal Preemption Controls | Phase 03 |
...
| Replanning | `GET` | `/incidents/{incident_id}/replan` | None | `ReplanEvaluationRead \| None` | None | 404 | Materiality Evaluation | Replan Banner / Dialog | Phase 03 |
| Replanning | `POST` | `/incidents/{incident_id}/replan/triggers` | `ReplanTriggerRequest` | `ReplanEvaluationRead` | `expected_incident_version` | 404, 409, 422 | Ingests obstruction/delay | Simulation / Traffic Feed | Phase 03 |
| Replanning | `POST` | `/incidents/{incident_id}/replan/evaluate` | `ReplanEvaluateRequest` | `ReplanEvaluationResponse` | `expected_incident_version` | 404, 409, 422 | Evaluates materiality | Dynamic Replanner | Phase 03 |
...
| Generate Corridor | `POST /api/v1/incidents/{id}/corridor` | `CorridorGenerateRequest` | None | `CorridorRead` (200) | Incident has no approved plan, resource not in active plan, or geometry missing | Refetch active plan routes | YES |
| Corridor Priority Update | `POST /api/v1/incidents/{id}/corridor/priority` | `CorridorPriorityRequest` | `expected_incident_version`, `expected_plan_version` | `CorridorRead` (200) | Stale incident or plan version | Refetch corridor state | YES |
...
| Ingest Replan Trigger | `POST /api/v1/incidents/{id}/replan/triggers` | `ReplanTriggerRequest` | `expected_incident_version` | `ReplanEvaluationRead` (200) | Stale incident version or unknown trigger reasons | Refetch incident; retry trigger | YES |
| Evaluate Replan | `POST /api/v1/incidents/{id}/replan/evaluate` | `ReplanEvaluateRequest` | `expected_incident_version` | `ReplanEvaluationResponse` (200) | Stale incident version or no active plan | Refetch incident and replan state | YES |
...
- `resource_type` / `type`: `AMBULANCE` | `FIRE_RESCUE`
- `status` / `operational_status`: `AVAILABLE` | `RESERVED` | `ASSIGNED` | `EN_ROUTE` | `ON_SCENE` | `TRANSPORTING` | `OUT_OF_SERVICE`
...
Returns `TrafficSnapshotRead` matching canonical backend fields exactly:
- `snapshot_id`: string
- `version`: int
- `provider_state`: string (e.g. `LIVE_FLOW_ACTIVE`, `FALLBACK_BASELINE`)
- `refresh_attempted_at`: ISO timestamp string
- `retrieved_at`: ISO timestamp string
- `provider_last_updated`: ISO timestamp string or `null`
- `freshness_status`: `FreshnessStatus` (`LIVE` | `FRESH` | `STALE` | `STATIC` | `UNKNOWN`)
- `source`: string (e.g. `tomtom_flow_api`)
- `source_reference`: string
- `data_reality`: `DataReality` (`REAL_LIVE` | `REAL_PUBLIC` | `SIMULATED` | `SYNTHETIC`)
- `flow_style`: string (`absolute`)
- `flow_zoom`: int (`22`)
- `units`: string (`kmph`)
...
- **UNKNOWN Fallback Policy:** If the traffic provider fails or feed expires, the backend marks `freshness_status = "UNKNOWN"` or `"STATIC"` and falls back to baseline OSM edge speeds. The frontend must not invent aggregate congestion ratios or sample point counts not provided by the model.

### 9.2 Replan State & Materiality
- **Inspection Endpoint:** `GET /api/v1/incidents/{incident_id}/replan` (returns `ReplanEvaluationRead | null`).
...
- **Materiality Policy:** Replan is marked `material = true` only if ETA increases by > 60s or > 15%, active route edge overlap drops below 80%, a required resource becomes unavailable, or an active route is blocked.
...
- `HospitalAcceptingState`: `ACCEPTING` | `NOT_ACCEPTING` | `UNKNOWN`.
...
| **Incident Rail** | Confidence Meter | `conf: 94` | `IncidentRead.confidence_level` | Render truthful categorical badge (`HIGH`, `MEDIUM`, `LOW`); NEVER fabricate numeric % | None | Phase 03 |
...
| **Resources Screen** | Resource Fleet Table | `MOCK_RESOURCES` | `GET /resources` | Map `ResourceRead` fields | Crew roster, fuel (mark presentation-only) | Phase 03 |
...
| `state/OperationsContext.tsx` | Global React context for operations | Partially compatible | Safe | **REUSE AS REFERENCE ONLY** | Rewrite cleanly: remove social/benchmark state; Phase 03 wires REST state only; WebSocket is deferred to Phase 05. |
