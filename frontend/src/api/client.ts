...
export async function request<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  if (text) {
    try {
      ...
      if (response.ok) {
        throw new ApiError(response.status, 'Backend returned an invalid response.', text);
      }
    }
  }

  if (!response.ok) {
    throw new ApiError(response.status, detailFromPayload(payload, response.status), payload);
  }
...
export function requireArray<T>(value: unknown, label: string): T[] {
  if (!Array.isArray(value)) {
    throw new ApiError(200, `Backend returned an invalid ${label} payload.`, value);
  }
  return value as T[];
}
