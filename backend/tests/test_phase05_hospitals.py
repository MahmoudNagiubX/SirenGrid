...
def test_farther_hospital_wins_on_simulated_burn_capability() -> None:
    """The demo must be able to show suitability beating proximity.

    Hospital A is nearer but has no confirmed burn capability. Hospital B is
    farther, carries an explicitly SIMULATED burn-care overlay, is accepting,
    and has a lighter modeled load. B must be recommended, and the explanation
    must record that the capability came from the simulated overlay rather
    than the real public registry.
    """
    hospitals = load_static_hospitals()
    ...
    assert winner["capability_penalty"] == 0.0
    assert winner["simulated_capability_tags"] == ["BURN_CARE"]

    loser = ranked[1].score_breakdown
    assert loser["capability_status"] == "UNKNOWN"
    assert loser["capability_source"] == "UNKNOWN"
    assert loser["capability_data_reality"] is None
    assert loser["capability_penalty"] == 0.5

    # The farther hospital wins despite the longer route.
    assert winner["route_eta_seconds"] > loser["route_eta_seconds"]
    assert ranked[0].score < ranked[1].score


def test_without_a_simulated_overlay_capability_remains_unknown() -> None:
    """No fixture means unknown; a demo overlay is never assumed."""
    hospitals = load_static_hospitals()
    hospital = hospitals[0]
    ...
    # Unknown is never converted into a confirmed absence, so the hospital is
    # still an option rather than being filtered out.
    assert len(ranked) == 1


def test_real_registry_capability_is_labelled_real_not_simulated() -> None:
    """A capability from the OSM registry must not be labelled simulated."""
    hospitals = load_static_hospitals()
    with_speciality = [h for h in hospitals if h.static_capabilities]
    assert with_speciality, "expected at least one OSM hospital with a speciality"

    hospital = with_speciality[0]
    required = (hospital.static_capabilities[0],)
