...
def serialize_zone_population_artifact(artifact: Mapping[str, Any]) -> bytes:
    """Return the canonical UTF-8/LF artifact bytes that the recorded hash locks."""
    return json.dumps(
...
def build_worldpop_provenance(
    ...
    """Return the integrity manifest for the published zone-population artifact.

    The WorldPop artifact is produced by this Phase 04 script rather than the
    Phase 02 OSM refresh, which rebuilds ``provenance.json`` from scratch and
    would drop any entry recorded there. It therefore carries its own manifest.
    """
...
def main() -> None:
    ...
    # Publish the integrity manifest only after the artifact it describes is in
    # place, so a failed publication never leaves a manifest for absent bytes.
